...
\item Añadir benchmarks para \texttt{assignVtoSlot()} y funciones concurrentes
\end{itemize}
...
func TestDupCarsWithSameIssue(t *testing.T) {
    ...
    if got1 != baseN {
        t.Fatalf("para %d coches esperábamos contador=%d; got=%d", 
                 baseN, baseN, got1)
    }
...
\subsection{types.go - Definición de Estructuras}
...
// Tipos enumerados para incidencias
type IssueType string
const (
...
// Tipos de identificadores (para mayor claridad semántica)
type ClientID int64
...
// Client representa a un cliente del taller
type Client struct {
    id       ClientID
    name     string
    phone    string
    email    string
    vehicles []VehicleID  // Relación 1:N con vehículos
}

// Mechanic representa un mecánico del taller
type Mechanic struct {
    id         MechanicID
    name       string
    skill      SkillType
...
// Mantiene relación con cliente (ownerID)
type Vehicle struct {
    ...
    ownerID   ClientID
    checkInAt time.Time
    eta       time.Duration  // Tiempo estimado de reparación
    issues    []IncidenceID
}
...
type Incidence struct {
    id          IncidenceID
    vehicleID   VehicleID
    mechanics   []MechanicID  // Múltiples mecánicos pueden trabajar
    kind        IssueType
    prio        Priority
    description string
    status      IssueStatus
...
// Slot representa una plaza física del taller
// Si vehicleID == nil, la plaza está libre
type Slot struct {
    number    int
    vehicleID *VehicleID
}

// HireRequest representa una solicitud de contratación
type HireRequest struct {
    vid   VehicleID       // Vehículo que necesita mecánico
    Reply chan *Mechanic  // Canal para devolver mecánico creado
}

// Garage es la estructura principal del sistema
type Garage struct {
    clients   []*Client
    vehicles  []*Vehicle
    mechanics []*Mechanic
    issues    []*Incidence
    slots     []*Slot
    vpool     []VehicleID      // Cola de vehículos esperando
    hirereqs  chan HireRequest // Canal para contratación
}
\end{lstlisting}

\subsection{main.go - Punto de Entrada}
...
func main() {
    ...
    for {
        // Revisar alertas de contratación
        g.checkAlert()
        ...
        case 1:  // Crear
            ...
            option = polyAskMenuInt("Seleccione qué crear: ", 8)
...
\subsection{new.go - Funciones de Creación (Extracto)}
...
func (g *Garage) newClient() {
    var c Client
    c.id = ClientID(askUniqueIntID("ID cliente (>0): ", 
        func(n int64) bool { return g.ownerIDexists(ClientID(n)) }))
    fmt.Printf("Nombre: ")
    fmt.Scanf("%s", &c.name)
    fmt.Printf("Teléfono: ")
    fmt.Scanf("%s", &c.phone)
...
func (g *Garage) newMech() *Mechanic {
    ...
    // Ampliar plazas del taller (2 por mecánico)
    g.genSlots()
    
    m.id = MechanicID(askUniqueIntID("ID del mecánico: ", 
        func(n int64) bool { return g.mechIDexists(MechanicID(n)) }))
    fmt.Printf("Nombre: ")
    fmt.Scanf("%s", &m.name)
    fmt.Printf("Experiencia(años): ")
    fmt.Scanf("%d", &m.experience)
    m.skill = SkillType(polyAskMenuStr("Especialidad:", 1))
    m.status = MechStatus(polyAskMenuStr("Estado:", 2))
...
\subsection{aux.go - Funciones Auxiliares (Extracto)}
...
// Verificar si un mecánico está libre
func isFree(m *Mechanic) bool {
    return len(m.issues) == 0 && m.status == ACTIVE
}

// Pedir un ID único de tipo entero
func askUniqueIntID(prompt string, exists func(int64) bool) int64 {
    var id int64
    for {
        fmt.Print(prompt)
...
// Revisar canal de contratación (no bloqueante)
func (g *Garage) checkAlert() {
    for {
        select {
        case req := <-g.hirereqs:
            fmt.Printf("-----------------------------------------------\n")
            fmt.Printf("[ALERTA] Contratar mecánico para %s.\n", req.vid)
            fmt.Printf("-----------------------------------------------\n\n")
            mech := g.newMech()
            req.Reply <- mech
            continue
...
// Función de testing: cuenta vehículos con una incidencia
...
// Función de testing: cuenta mecánicos por especialidad
...
\subsection{modify.go - Asignación Concurrente (Extracto Clave)}
...
// Meter un vehículo a una plaza del taller
func (g *Garage) addV() {
    var vid VehicleID
    
    if g.availableSlots() == 0 {
        fmt.Printf("No hay plazas disponibles.\n\n")
        return
    }
    ...
    vid = VehicleID(askUniqueStrID("Matrícula del vehículo: ", 
        func(s string) bool { return !g.vidExists(VehicleID(s)) }))
    
    // Lanzar goroutine para asignación concurrente
    go g.assignVtoSlot(vid)
}

// Función principal de asignación (goroutine)
func (g *Garage) assignVtoSlot(vid VehicleID) {
    ...
    // 1) Obtener mecánico principal
    mainMech := g.obtainMechanicForVehicle(vid)
    
    // 2) Meter vehículo en plaza
    ...
    // 7) Cerrar incidencias y liberar
    closeIssues(issues)
...
// Espera activa hasta mecánico libre
func (g *Garage) waitForFreeMech() *Mechanic {
    var m *Mechanic
    for {
        m = g.getFreeMech()
...
// Construir slice de incidencias y calcular ETA
func (g *Garage) collectIssuesAndEta(v *Vehicle) ([]*Incidence, time.Duration) {
    var (
        issues []*Incidence
        eta    time.Duration
...
// Obtener segundo mecánico (o contratar)
...
\textbf{1. Diseño de Tipos Personalizados:}

El uso de tipos como \texttt{ClientID}, \texttt{MechanicID}, \texttt{VehicleID} mejoró significativamente la legibilidad del código y previno errores de mezcla de identificadores.

\textbf{2. Canales como Mecanismo de Comunicación:}
...
\textbf{Mejoras de Corto Plazo:}
\begin{itemize}
\item Implementar persistencia con codificación JSON
\item Añadir mutexes para acceso seguro a estructuras compartidas
\item Ampliar suite de tests con tests de integración
\item Validación avanzada de entrada con expresiones regulares
\end{itemize}

\textbf{Mejoras de Medio Plazo:}
\begin{itemize}
\item Interfaz web con servidor HTTP
\item API REST para integración con otros sistemas
\item Base de datos relacional (SQLite o PostgreSQL)
\item Sistema de autenticación y roles de usuario
\end{itemize}

\textbf{Mejoras de Largo Plazo:}
\begin{itemize}
\item Dashboard en tiempo real con WebSockets
\item Notificaciones push a clientes
\item Sistema de facturación integrado
\item Análisis de datos y métricas de rendimiento
\end{itemize}
...
\subsection{Apéndice A: Estructura del Proyecto}
...
\subsection{Apéndice C: Configuración de LyX}

Para importar este documento en LyX:

\begin{enumerate}
\item Guardar el archivo .tex generado
\item Abrir LyX
\item \textbf{Archivo → Importar → LaTeX (Simple)}
\item Seleccionar el archivo .tex
\item Verificar que se importó correctamente
\item \textbf{Archivo → Exportar → PDF (pdflatex)}
\end{enumerate}

\textbf{Paquetes necesarios en LaTeX:}
\begin{itemize}
\item inputenc, babel (español)
\item amsmath, amsfonts, amssymb
\item listings, xcolor
\item graphicx, hyperref
\item geometry
\end{itemize}
